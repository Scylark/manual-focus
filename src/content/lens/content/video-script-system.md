...
**Phase 1, brief intake.** Required fields are video purpose (educate, convince, launch, entertain), length (30s, 60s, 3min, 8min, formats differ meaningfully), platform mix, the single key takeaway, the spokesperson, and the proof or example.

**Phase 2, beat sheet.** The model generates a structured beat sheet appropriate to the length. A 60-second video has 4 to 6 beats. An 8-minute video has 12 to 18 beats. Each beat has a time mark, on-screen action, dialogue (with timing), B-roll opportunity, and on-screen text.
...
**Phase 4, full script.** Each beat is expanded with dialogue, action notes, and timing. The script is in production format (talent's lines clearly marked, B-roll in brackets, on-screen text in caps). Designed to be shootable as-is.

**Phase 5, asset briefs.** Companion outputs are 3 to 5 thumbnail concepts for design, a caption draft for the post, and a shortlist of B-roll the producer should source ahead of the shoot.

## The eval gates

**Eval 1, length adherence.** Spoken-word count maps to time at around 150 words per minute. If the script's word count implies a runtime more than 15% over target, it's too long. Trim. (Most first-draft scripts run long.)
...
**Talent goes off-script.** Some talent improvises. If the talent is good, this is fine. If they're not, you've lost the script's discipline. Build a "freeze words" list of phrases the talent must hit verbatim, and accept improvisation in between.
...
Illustrative scenarios that show common shapes a video-script system takes. Specifics are illustrative and the patterns repeat.
