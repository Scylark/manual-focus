...
The dishonest answer — "depends on what you do with it" — is what most marketing teams give, and it's why marketing keeps losing budget arguments to operations and product.

This playbook is the modelling work, done properly, in two days. It fits diminishing-return curves to each of your active channels using the last 12 months of weekly data, then simulates 20 allocation scenarios with confidence intervals on each. You walk into the budget meeting with three defensible options and the maths behind them, not a vibe.

This is not a media-mix-model. MMM is bigger work that needs 18+ months of data and a lot of econometric care. This is a lighter, faster, channel-mix-simulator built for the weekly-budget conversation, not the annual one.

## The pipeline

Five phases.

**Phase 1 — Data shape.** Pull weekly spend, weekly conversions and weekly cost-per-action for each active paid channel from the last 12 months. For organic channels (SEO, owned), use weekly clicks/sessions and conversions. Normalise into a long-format table: `channel, week, spend, output, cost`. The model needs structured input, not screenshots.

**Phase 2 — Curve fitting.** For each channel, fit a Hill function (saturation curve) to the spend-output pairs. Three parameters per channel: half-saturation point, max output, current operating point. The Hill function is a good fit for most performance channels because it captures the diminishing-return behaviour humans intuit but rarely quantify.

**Phase 3 — Cross-channel adjustment.** Apply assumed cross-channel multipliers (brand drives search, paid social warms email, etc.). These are bounded assumptions, not derived — the pipeline asks you to set them explicitly with high/low estimates and uses both bounds in the scenarios. We ship a default starter set drawn from MMM literature.
...
**Eval 4 — Recommendation realism.** The pipeline-suggested optimal allocation often wants to zero out small channels. We force a floor: no channel can be cut below 60% of its 12-month average in a single quarter without explicit override. Saves the simulator from suggesting "kill SEO" allocations that ignore the multi-quarter lag.

## The failure modes

**Saturation isn't real on every channel.** Some channels (especially long-tail SEO, organic content) don't saturate in the data you have because you've never spent enough to find the ceiling. The Hill fit will still produce a curve, but it's extrapolating beyond what the data supports. The pipeline labels these "headroom unknown" and the recommendation language treats them as upside, not a target.

**Cross-channel multipliers are guesses.** The default multipliers we ship are from MMM literature. They're better than nothing and worse than your own MMM. If you have a recent MMM, override the defaults with your actual lifts. If you don't, run two scenarios — one with the defaults at high estimate, one at low — and present both as the band.

**Lag isn't priced in for SEO.** Cutting SEO budget shows up in conversions 6 to 12 months later. A 90-day projection won't see the damage. The pipeline applies a strategic floor on SEO by default (60% of trailing average) and flags the recommendation explicitly when SEO is touched. This is a guardrail, not a model, because the model can't see that far forward.
...
**D2C apparel, growth-stage.** Brand was about to triple Meta spend on the back of a strong quarter. Pipeline showed Meta was already 80% of the way up its saturation curve. The recommendation was to take 35% of the planned increase and put it into a partner-PR push that had only ever run at a small spend. They did it. Quarter beat the plan with the recommended split, and the partner channel had a curve to climb for the following two quarters.

**B2B SaaS, scale-stage.** CFO wanted to cut SEO spend in half because "we can't see what it's doing." Pipeline showed the brand was in the steep part of the SEO curve (small spend increases yielding outsized organic gains) and that the lag on cuts would land 9 months out, the same week the new product launch was scheduled. We recommended the opposite: a modest increase, but reallocate within SEO from low-intent informational pages to high-intent commercial ones. CFO accepted. Nine months later, the product launch landed into the strongest organic quarter the brand had ever had.

**Fintech, retired engagement.** Pipeline V1 didn't apply the strategic floor on email lifecycle, recommended cutting email spend by 60%. The team did it. Two quarters later, the database had decayed enough that re-warming it took 9 months. We added the floor logic off this engagement (V2). The lesson: any channel with an audience-asset (email list, SEO authority, brand recall) needs a strategic floor that the model can't override.
