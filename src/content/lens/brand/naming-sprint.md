...
stack: brand
description: "Generate a category-aware shortlist of brand names with phonetic checks, cultural sweeps and trademark-risk first-pass. Cut from 400 candidates to 8 in one day."
...
You still need a human to pick. The pipeline gets you to eight defensible candidates in a day rather than fifty fragile ones in a quarter.
...
**Pass 2, generation.** Run six generation strategies in parallel, each producing 50 candidates. Strategies include invented compound, modified real word, archaic revival, two-syllable abstract, metaphor-anchored, and competitor-contrarian. Generation is cheap, so over-produce. Aim for 300+ candidates total.
...
**Pass 4, distinctiveness scoring.** For each survivor, generate the semantic embedding and compare distance against the embeddings of the three named competitors plus the top ten brands in the broader category. Score 0 to 10 on category distance. Anything below 4 is too close.

**Pass 5, trademark first-pass.** Hit USPTO TESS, EUIPO and IPO via API for exact and near matches in the relevant trademark classes. This is a first-pass and does not replace a trademark attorney. Names with exact matches in the same class die. Names with same-class similars get flagged for legal review.
...
The generation prompts run in parallel. Here is the **invented compound** strategy, which tends to produce the highest hit rate.
...
**Eval 1, generation diversity.** Run the six strategies and check that the 300 candidates produce a vocabulary diversity score (unique 3-grams / total 3-grams) above 0.75. Below that, the model is repeating itself, so bump temperature, switch model, or rotate seed.

**Eval 2, phonetic accuracy.** Random sample 20 candidates and verify the IPA against a phonetician's transcription. A reference set of 200 names with known IPA can support this. Accuracy target 90%+. Below 85% means the model is hallucinating phonetics, so fall back to a dedicated phonetic library.

**Eval 3, trademark recall.** Salt the candidate list with 5 names known to be registered. Pass 5 should catch all 5. Catching 4/5 is acceptable. 3/5 means the API integration is dropping classes, so check the class mapping in the request.

**Eval 4, strategy fit.** Have the brand's lead reviewer rate the top 12 on strategy fit (1 to 5). At least 8 of 12 should score 4 or above. If only 4 or 5 do, the strategy intake captured the wrong inputs, so re-run with sharper connotation lists.
...
**You forgot the domain.** The pipeline doesn't check domain availability by default because it's noisy data (some are listed but unsold, some sit on registrar squat lists). A separate Pass 7 covers domain checks for engagements where domain matters at launch.

## The pattern in practice

Illustrative scenarios that show common shapes the sprint takes. Specifics are illustrative and the patterns repeat.
...
**D2C wellness brand, full rebrand.** Twelve founders, half of them with strong opinions. The sprint produces the top 8 in a day, and blind rationale review in a 90-minute session usually picks unanimously. The honest read is that without the sprint, four rounds of opinion-led elimination would have run. With it, pre-screening removes the names that would have triggered the debates.
