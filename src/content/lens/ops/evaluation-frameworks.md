...
The single most-asked question in AI-leveraged marketing teams is whether a given output is good. Without an answer, the team either ships every draft (and the brand suffers), or hand-reviews every draft (and the time saving evaporates). The only stable path is objective evaluation, with rubrics that score the output independent of who's looking at it.

This playbook installs an eval discipline. It's a library of rubrics, one per output type the team produces (headline, body copy, ad creative, email, social post, deck slide). Each rubric is calibrated against a corpus of known-good and known-bad examples. Each rubric tells you whether to use a deterministic check, an LLM-as-judge, or both. The result is the team scoring its own output the way the brand lead would, in under a second per piece.
...
**Phase 2, rubric draft.** From the tagging, extract the criteria that the brand lead is actually applying. Most teams discover the lead is using 5-8 criteria, rather than the 30 in the brand guide. The rubric is built from what gets used, rather than what's documented.

**Phase 3, check-type selection.** For each criterion, decide between deterministic (regex, count, length, structural), LLM judgement (with a prompt and a model pinned), or hybrid (deterministic first, LLM only if deterministic passes). Default to deterministic where possible, because deterministic checks are fast, free, and repeatable.

**Phase 4, calibration.** Run the draft rubric against the 30-example corpus. Check correlation with the brand lead's tags. Target sits above 0.8 correlation with the lead's known-good and known-bad split. Below that, the rubric is missing criteria or applying them wrong, so adjust.

**Phase 5, production install.** Wire the rubric into the team's drafting pipeline (or the brand-guardrails system from that playbook). Pass means promote. Borderline means flag for human. Fail means regenerate with the failing criteria named.

## The four rubric types

**Type A, deterministic structural.** Counts, lengths, regex. Examples include headline at 65 chars or fewer, no exclamation marks, primary keyword in first 100 words, paragraph max 75 words. Fast, free, no LLM. Pass/fail per criterion, sum to a score.

**Type B, deterministic stylometric.** Pattern matching against the voice profile. Sentence length distribution, contraction count, transition word usage, opener pattern. From the brand-voice playbook. Around 30ms per piece.
...
Most output types use a mix. A blog post evaluation runs structural (Type A), voice (Type B), originality vs SERP (Type C), and claim plausibility (Type C). The rubric file specifies which checks apply, in what order, and what the gates are.

## The LLM-as-judge prompt pattern

The pattern that holds up. Avoid asking the judge model "is this good?", which produces drift. Ask specific questions with the criteria explicit.
...
**Eval-of-eval 2, inter-rater agreement.** For LLM-judged criteria, run two judge models in parallel for 20 pieces. They should agree on the score within ±1 point for 80%+ of pieces. If they disagree more, the criterion definition is ambiguous, so sharpen it.

**Eval-of-eval 3, human-baseline correlation.** Quarterly, the brand lead independently scores 20 pieces. Correlation with the rubric output should stay above 0.7. Below that, the rubric has drifted from the lead's actual judgement and needs recalibration.
...
**Ambiguity flags get ignored.** When the judge flags ambiguity, that's a signal the criterion or the example isn't ready for judgement. Flagged outputs should go to human review, rather than getting pushed through with a default score.

**The rubric becomes the work.** Some teams optimise for the rubric rather than the outcome. If headlines pass the rubric but stop performing in production, the rubric has drifted from the goal. Tie the rubric calibration back to actual performance every quarter, so pieces that scored well should be outperforming pieces that scored poorly.
...
Illustrative scenarios that show common shapes evaluation frameworks take. Specifics are illustrative and the patterns repeat.
